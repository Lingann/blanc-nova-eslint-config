...
export default {
  ...
  rules: {
    ...
    // 'no-redeclare': 'off', // 禁止重复声明
    // 'no-use-before-define': 'off', // 禁止在变量定义之前使用它们
    // 'brace-style': 'off', // 大括号风格
    // 'comma-dangle': 'off', // 逗号风格
    // 'object-curly-spacing': 'off', // 大括号内空格
    // 'semi': 'off', // 分号
    // 'quotes': 'off', // 引号
    // 'space-before-blocks': 'off', // 块前空格
    // 'space-before-function-paren': 'off', // 函数括号前空格
    // 'space-infix-ops': 'off', // 运算符前后空格
    // 'keyword-spacing': 'off', // 关键字前后空格
    // 'comma-spacing': 'off', // 逗号前后空格
    // 'no-extra-parens': 'off', // 禁止不必要的括号
    // 'no-dupe-class-members': 'off', // 禁止类成员中出现重复的名称
    // 'no-loss-of-precision': 'off', // 禁止数字字面量中使用前导和末尾小数点
    // 'lines-between-class-members': 'off', // 要求或禁止在类成员之间出现空行
  },
}
